...
function Fourbooking() {
  ...
  useEffect(() => {
    const fetchVehicles = async () => {
      try {
        const response = await axios.get('http://localhost/vechilebackend/controllers/api/User/Get/fourimage.php'); 
        ...
        {vehicles.length > 0 ? (
          vehicles.map((vehicle) => (
            <div className="vehicle-card" key={vehicle.id}>
              ...
              <Link to={{pathname: `/fourbook/${vehicle.id}`,}}state={{ price: vehicle.price }} 
>
