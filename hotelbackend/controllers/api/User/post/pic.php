...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['file'])) {
        ...
        $name = $_POST['name'] ?? '';
        $price = $_POST['price'] ?? '';
        $max = $_POST['max'] ?? '';  // Fixed: Changed $brand to $max

        if (empty($name) || empty($price) || empty($max)) {  // Ensure that $max is checked
            handleResponse(400, 'Invalid input. Please provide name, price, and max.');
        }

        $obj = new Post();
        $result = $obj->images($name, $price, $max, $uploadPath);  // Pass $max to the function
